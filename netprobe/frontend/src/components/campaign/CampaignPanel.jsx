...
export function CampaignPanel({ ws, selectedModules, intensity, duration }) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4" style={{ background:'rgba(0,0,0,0.2)' }}>
      <div className="w-full space-y-4">
        ...
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center"
            ...
            <Target className="w-4.5 h-4.5 text-[#0a84ff]" />
          </div>
          <div>
            <h2 className="text-sm font-bold text-white">Campaña Multi-Target</h2>
            <p className="text-[10px] text-[rgba(255,255,255,0.35)]">Escanea múltiples IPs con el mismo perfil de módulos</p>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-[rgba(255,255,255,0.09)] p-4 space-y-3"
          ...
          <p className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.35)] font-semibold">Targets</p>
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input value={inputVal} onChange={e => { setInputVal(e.target.value); setInputError(''); }}
                ...
                className="w-full bg-[rgba(255,255,255,0.05)] border border-[rgba(255,255,255,0.1)] rounded-lg px-3 py-2 text-xs text-white outline-none focus:border-[rgba(10,132,255,0.4)] placeholder-[rgba(255,255,255,0.2)]" />
              {inputError && (
                <p className="text-[9px] text-[#ff9f0a] mt-1 flex items-center gap-1">
            ...
            <button onClick={addTarget} disabled={!inputVal.trim()}
              ...
              style={{ background:'rgba(10,132,255,0.15)', border:'1px solid rgba(10,132,255,0.3)', color:'#0a84ff' }}>
              <Plus className="w-4 h-4" />
            </button>
            <label className="px-3 py-2 rounded-lg text-xs font-semibold cursor-pointer transition-all hover:bg-[rgba(255,255,255,0.06)]"
                ...
                {targets.map(ip => {
                  ...
                  return (
                    <motion.div key={ip}
                      ...
                      style={{
                        background: res ? `${scoreColor(res.score)}18` : 'rgba(255,255,255,0.06)',
                        border: `1px solid ${res ? scoreColor(res.score)+'40' : 'rgba(255,255,255,0.1)'}`,
                        color: res ? scoreColor(res.score) : 'rgba(255,255,255,0.7)',
                      }}>
                      {running && currentTgt === ip && <div className="w-1.5 h-1.5 rounded-full bg-[#0a84ff] animate-pulse" />}
                      {res && <CheckCircle2 className="w-3 h-3" />}
                      {ip}
                      {res && <span className="text-[9px] font-bold">{res.score}/100</span>}
                      {!running && (
                        <button onClick={() => removeTarget(ip)}
                          className="text-[rgba(255,255,255,0.3)] hover:text-[#ff453a] transition-colors">
                          <X className="w-3 h-3" />
                        </button>
                      )}
                    </motion.div>
        ...
        <div className="rounded-xl border border-[rgba(255,255,255,0.09)] p-4 space-y-3"
          style={{ background:'rgba(255,255,255,0.02)' }}>
          <p className="text-[10px] uppercase tracking-widest text-[rgba(255,255,255,0.35)] font-semibold">Configuración</p>

          <div className="flex gap-2">
            <button onClick={() => setUseProfile(false)}
              className="flex-1 py-2 rounded-lg text-xs font-medium transition-all"
              style={{
                background: !useProfile ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${!useProfile ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.07)'}`,
                color: !useProfile ? 'white' : 'rgba(255,255,255,0.4)',
              }}>
            ...
            <button onClick={() => setUseProfile(true)}
              className="flex-1 py-2 rounded-lg text-xs font-medium transition-all"
              style={{
                background: useProfile ? 'rgba(0,255,136,0.08)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${useProfile ? 'rgba(0,255,136,0.25)' : 'rgba(255,255,255,0.07)'}`,
                color: useProfile ? '#00ff88' : 'rgba(255,255,255,0.4)',
              }}>
              Usar perfil
            </button>
          </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {PROFILES.map(p => (
                <button key={p.id} onClick={() => setProfile(p.id)}
                  ...
                  style={{
                    background: profile === p.id ? `${p.color}15` : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${profile === p.id ? p.color+'40' : 'rgba(255,255,255,0.07)'}`,
                    color: profile === p.id ? p.color : 'rgba(255,255,255,0.5)',
                  }}>
                  <span>{p.icon}</span>
          ...
          <div className="flex items-center gap-3 text-[10px] text-[rgba(255,255,255,0.35)] pt-1">
        ...
        <button
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(10,132,255,0.25), rgba(0,255,136,0.15))',
            border: '1px solid rgba(10,132,255,0.4)',
            color: running ? '#0a84ff' : 'white',
          }}>
          {running ? (
        ...
        {Object.keys(campaignRes).length > 0 && (
          <div className="rounded-xl border border-[rgba(255,255,255,0.09)] overflow-hidden"
            style={{ background:'rgba(255,255,255,0.02)' }}>
            <div className="px-4 py-2.5 border-b border-[rgba(255,255,255,0.07)]">
              <p className="text-xs font-semibold text-white">Resultados de campaña</p>
            </div>
            {Object.entries(campaignRes).map(([ip, res]) => (
              <div key={ip} className="flex items-center gap-3 px-4 py-2.5 border-b border-[rgba(255,255,255,0.05)] last:border-0">
