...
export function Report({ results, target, globalScore, onGenerateAIReport }) {
  ...
  if (!results.length) {
    return (
      <div className="h-full flex items-center justify-center text-[rgba(255,255,255,0.2)]">
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={onGenerateAIReport}
            ...
            style={{ background:'rgba(0,132,255,0.1)', border:'1px solid rgba(10,132,255,0.3)', color:'#0a84ff' }}>
          ...
          <button onClick={exportPDF} disabled={pdfLoading}
            ...
            style={{ background: pdfDone ? 'rgba(48,209,88,0.15)' : 'rgba(255,69,58,0.12)',
                     border:`1px solid ${pdfDone ? 'rgba(48,209,88,0.4)' : 'rgba(255,69,58,0.35)'}`,
                     color: pdfDone ? '#30d158' : '#ff453a' }}>
      ...
      {pdfError && (
        <div className="text-xs text-[#ff453a] bg-[rgba(255,69,58,0.08)] border border-[rgba(255,69,58,0.25)] rounded-lg px-3 py-2">
          {pdfError}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
        <div className="sm:col-span-1 rounded-xl border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)] p-4 flex flex-col items-center gap-1">
          <div className="text-3xl font-bold" style={{ color: getScoreColor(globalScore) }}>{globalScore}</div>
          <div className="text-[9px] text-[rgba(255,255,255,0.35)]">/ 100</div>
        ...
        {Object.entries(STATUS_CONFIG).map(([status, cfg]) => {
          ...
          return (
            <div key={status} className="rounded-xl border p-3 flex flex-col items-center gap-1"
              ...
              <span className="text-[8px] text-[rgba(255,255,255,0.35)] uppercase tracking-wider">{cfg.label}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="rounded-xl border border-[rgba(255,255,255,0.08)] overflow-hidden">
        <div className="px-4 py-2.5 bg-[rgba(255,255,255,0.02)] border-b border-[rgba(255,255,255,0.07)]">
        ...
        {sorted.map((r, i) => {
          ...
          return (
            <motion.div key={r.id} initial={{ opacity:0 }} animate={{ opacity:1 }}
              ...
              className="flex items-center gap-3 px-4 py-2.5 border-b border-[rgba(255,255,255,0.04)] last:border-0">
      ...
      <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[rgba(255,255,255,0.02)] p-4">
        <p className="text-[9px] text-[rgba(255,255,255,0.3)] leading-relaxed">
          <span className="font-semibold text-[rgba(255,255,255,0.5)]">Exportar PDF</span> — genera un informe profesional con portada,
